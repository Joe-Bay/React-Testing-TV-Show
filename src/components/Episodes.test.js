...
test('renders the data from the API call', () => {

    const { rerender } = render(<Episodes episodes={[]}/>)

    let showArr = screen.queryAllByTestId(/shows/i)
    expect(showArr).toHaveLength(0)

    rerender(<Episodes episodes={showData}/>)
    showArr = screen.getAllByTestId(/shows/i)
    expect(showArr).toHaveLength(8)
